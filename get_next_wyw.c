...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/14 17:56:23 by bspindle          #+#    #+#             */
/*   Updated: 2017/12/14 17:58:17 by bspindle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_next_wyw(const int fd, char **line, char delim)
{
	...
	if (!line || fd < 0 || fd >= OPEN_MAX)
		return (-1);
	if (!(save[fd].size > 0 &&
				ft_strnchr(save[fd].read_part, delim, save[fd].size)))
		i = ft_read_file(fd, &save[fd], delim);
	if (i == -1)
		return (-1);
	i = ft_fill_line(&save[fd], line, delim);
	return (i);
}
